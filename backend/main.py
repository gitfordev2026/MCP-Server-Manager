...
async def lifespan(app: FastAPI):
    ...
    if mcp_asgi_app is not None and hasattr(mcp_asgi_app, "router") and hasattr(mcp_asgi_app.router, "lifespan_context"):
        # Wrap the FastMCP inner ASGI application's lifespan natively
        async with mcp_asgi_app.router.lifespan_context(app):
            yield
        return

    # Fallback to older `mcp` SDK built-in logic
...
async def _fetch_all_mcp_server_resources() -> dict[str, tuple[str, str, Any]]:
    import asyncio as _aio
    with SessionLocal() as db:
        rows = db.scalars(select(ServerModel).where(ServerModel.is_deleted == False, ServerModel.is_enabled == True)).all() # noqa: E712
    ...
    async def _probe(name: str, url: str) -> list[tuple[str, str, Any]]:
        client = None
        try:
            client = MCPClient({"mcpServers": {name: {"url": url}}})
            await client.create_all_sessions()
            session = client.get_session(name)
            resources_result = await _aio.wait_for(session.list_resources(), timeout=10.0)
            return [(f"mcp__{name}__{r.name if hasattr(r, 'name') and r.name else r.uri}", name, r.uri, r) for r in getattr(resources_result, "resources", []) or getattr(resources_result, "__root__", []) or [] if hasattr(r, 'uri')]
        except Exception as exc:
            print(f"[combined-mcp] Could not list resources for '{name}': {exc}")
            return []
        finally:
            if client:
                try: await client.close_all_sessions()
    ...
    for batch in results:
        ...
        for prefixed_name, srv_name, orig_uri, res_obj in batch:
            result[prefixed_name] = (srv_name, orig_uri, res_obj)
    return result

async def _fetch_all_mcp_server_prompts() -> dict[str, tuple[str, str, Any]]:
    import asyncio as _aio
    with SessionLocal() as db:
        rows = db.scalars(select(ServerModel).where(ServerModel.is_deleted == False, ServerModel.is_enabled == True)).all() # noqa: E712
        servers = [(row.name, row.url) for row in rows]
    ...
    async def _probe(name: str, url: str) -> list[tuple[str, str, Any]]:
        ...
        try:
            ...
            await client.create_all_sessions()
            session = client.get_session(name)
            prompts_result = await _aio.wait_for(session.list_prompts(), timeout=10.0)
            return [(f"mcp__{name}__{p.name}", name, p.name, p) for p in getattr(prompts_result, "prompts", []) or getattr(prompts_result, "__root__", []) or []]
        except Exception as exc:
            print(f"[combined-mcp] Could not list prompts for '{name}': {exc}")
            return []
        finally:
            if client:
                try: await client.close_all_sessions()
    ...
    for batch in results:
        if isinstance(batch, BaseException): continue
        for prefixed_name, srv_name, orig_name, pr_obj in batch:
            result[prefixed_name] = (srv_name, orig_name, pr_obj)
    return result
...
class CombinedAppsOpenAPIMCP(FastMCP[Any]):
    ...
    async def list_resources(self) -> list[Any]:
        ...
        mcp_resources = await _fetch_all_mcp_server_resources()
        resources = []
        for prefixed_name, (server_name, orig_uri, res_obj) in mcp_resources.items():
            resources.append(
                MCPResource(
                    uri=f"mcp__{server_name}__{orig_uri}",
                    name=getattr(res_obj, "name", prefixed_name),
                    mimeType=getattr(res_obj, "mimeType", getattr(res_obj, "mime_type", "text/plain")),
                    description=f"[MCP: {server_name}] {getattr(res_obj, 'description', '') or 'No description'}",
                )
            )
        return resources

    async def read_resource(self, uri: Any) -> list[Any]:
        # FastMCP standardizes URI as AnyUrl or str
        uri_str = str(uri)
        if not uri_str.startswith("mcp__"):
            raise ValueError(f"Unknown resource prefix: {uri_str}")
        
        parts = uri_str.split("__", 2)
        if len(parts) != 3:
            raise ValueError(f"Malformed resource URI: {uri_str}")
        server_name, orig_uri = parts[1], parts[2]
        ...
        client = None
        try:
            client = MCPClient({"mcpServers": {server_name: {"url": server.url}}})
            await client.create_all_sessions()
            session = client.get_session(server_name)
            result = await session.read_resource(orig_uri)
            # return what the client returned, which is Iterable[ReadResourceContents] or something similar
            return getattr(result, "contents", []) or getattr(result, "__root__", getattr(result, "content", []))
        finally:
            if client:
                try: await client.close_all_sessions()
                except Exception: pass
    ...
    async def list_prompts(self) -> list[Any]:
        ...
        mcp_prompts = await _fetch_all_mcp_server_prompts()
        prompts = []
        for prefixed_name, (server_name, orig_name, pr_obj) in mcp_prompts.items():
            prompts.append(
                MCPPrompt(
                    name=prefixed_name,
                    description=f"[MCP: {server_name}] {getattr(pr_obj, 'description', '') or 'No description'}",
                    arguments=getattr(pr_obj, "arguments", []),
                )
            )
        return prompts
    ...
    async def get_prompt(self, name: str, arguments: dict[str, Any] | None = None) -> Any:
        ...
        if len(parts) != 3:
            raise ValueError(f"Malformed prompt name: {name}")
        server_name, orig_name = parts[1], parts[2]
        ...
        try:
            ...
            await client.create_all_sessions()
            session = client.get_session(server_name)
            return await session.get_prompt(orig_name, arguments or {})
        finally:
            if client:
                try: await client.close_all_sessions()
                except Exception: pass
...
if __name__ == "__main__":
    uvicorn.run("main:app", host="0.0.0.0", port=8091, reload=True, ws="wsproto")
