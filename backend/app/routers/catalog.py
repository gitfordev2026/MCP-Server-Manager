...
def create_catalog_router(
    ...
    async def get_openapi_tool_catalog(
        ...
        healthy_count = sum(
            ...
            1 for server in active_servers
            if str(getattr(server, "last_sync_status", "never")) != "failed"
        )
        unreachable_count = sum(
            1 for app in active_apps if str(getattr(app, "last_sync_status", "never")) == "failed"
        ) + sum(
            1 for server in active_servers if str(getattr(server, "last_sync_status", "never")) == "failed"
        )
        ...
        apps = [
            ...
            for app in list(active_apps) + list(active_servers)
        ]
        sync_errors = [
