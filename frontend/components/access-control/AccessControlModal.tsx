    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title={`Access Control: ${ownerId}`}>
            <div className="flex bg-slate-100 dark:bg-slate-800 rounded-xl p-1 mb-4">
                <button
                    className={`px-4 py-2 text-sm font-semibold rounded-lg transition-colors ${activeTab === 'default' ? 'bg-white dark:bg-slate-900 text-violet-700 shadow-sm' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:text-slate-300'}`}
                    onClick={() => setActiveTab('default')}
                >
                    Default Policy
                </button>
                <button
                    className={`px-4 py-2 text-sm font-semibold rounded-lg transition-colors ${activeTab === 'endpoints' ? 'bg-white dark:bg-slate-900 text-violet-700 shadow-sm' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:text-slate-300'}`}
...
function DefaultPolicyForm({ ownerId, policy }: { ownerId: string; policy: OwnerPolicy }) {
    ...
    return (
        <div className="space-y-4">
            <div>
                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300">Default Access Mode</label>
                <select
                    value={mode}
                ...
                <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">Applies to all tools unless overridden.</p>
            </div>

            <div>
                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300">Allowed Users (emails)</label>
                <textarea
                    value={users}
            ...
            <div>
                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300">Allowed Groups</label>
                <input
                    type="text"
                    value={groups}
    ...
    return (
        <div className="space-y-4">
            {endpointIds.map((toolId) => {
                ...
                return (
                    <div key={toolId} className="border rounded-lg p-3 transition-colors hover:bg-slate-50 dark:bg-slate-800/50">
                        <div className="flex justify-between items-center mb-2">
                            <div className="flex items-center gap-2 min-w-0">
                                <span className="font-medium text-sm truncate" title={toolId}>{toolId}</span>
                                {!hasOverride && (
                                    <span className="inline-block px-2 py-0.5 rounded text-[11px] font-medium bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400">
                                        inherited
                                    </span>
                                )}
                            </div>
...
function EndpointForm({
    ...
    return (
        <div className="space-y-3 bg-white dark:bg-slate-900 p-3 rounded border border-blue-100 animate-in fade-in zoom-in-95 duration-150">
            <div>
                <label className="block text-xs font-medium text-slate-700 dark:text-slate-300">Access Mode</label>
                <select
                    value={mode}
                    onChange={(e) => setMode(e.target.value as AccessMode)}
            ...
            <div>
                <label className="block text-xs font-medium text-slate-700 dark:text-slate-300">Allowed Users</label>
                <textarea
                    value={users}
                    onChange={(e) => setUsers(e.target.value)}
                    className="mt-1 block w-full rounded border-gray-300 text-sm p-1"
            ...
            <div>
                <label className="block text-xs font-medium text-slate-700 dark:text-slate-300">Allowed Groups</label>
                <input
                    type="text"
                    value={groups}
                    onChange={(e) => setGroups(e.target.value)}
