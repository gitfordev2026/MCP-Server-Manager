  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-white via-slate-50 to-slate-100 dark:from-slate-950 dark:via-slate-900 dark:to-slate-950 overflow-hidden">
      ...
      <main className="pt-24 pb-12 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
        ...
        {/* Spec Info */}
        {spec?.info && (
          <Card className="bg-white dark:bg-slate-900/70 backdrop-blur-xl border border-amber-300 dark:border-amber-700/50 mb-8 p-6 shadow-lg shadow-amber-200/30">
        ...
        {/* Loading State */}
        {loading && (
          <Card className="bg-white dark:bg-slate-900/70 backdrop-blur-xl border border-slate-200 dark:border-slate-700/50 p-12 text-center shadow-lg">
        ...
        {!loading && !error && pathItems.length === 0 && (
          <Card className="bg-white dark:bg-slate-900/70 backdrop-blur-xl border border-slate-200 dark:border-slate-700/50 p-8 text-center shadow-lg">
              ...
              <tbody>
                {pathItems.map((item, idx) =>
                  item.methods.map((method, methodIdx) => (
                    <tr
                      key={`${idx}-${methodIdx}`}
                      className="border border-slate-200 dark:border-slate-700/50 hover:bg-white dark:bg-slate-900/50 transition-colors cursor-pointer"
                      onClick={() =>
                        setSelectedMethod(
                    ...
                    >
                      {/* Method */}
                      <td className="border border-slate-200 dark:border-slate-700/50 px-6 py-4">
                        <span
                      ...
                      {/* Endpoint */}
                      <td className="border border-slate-200 dark:border-slate-700/50 px-6 py-4">
                        <p className="text-slate-800 dark:text-slate-200 font-mono text-sm break-all text-blue-600">{item.path}</p>
                      </td>

                      {/* Description */}
                      <td className="border border-slate-200 dark:border-slate-700/50 px-6 py-4">
                        <div>
                          {method.summary && <p className="text-slate-700 dark:text-slate-300 text-sm font-medium">{method.summary}</p>}
                      ...
                      {/* Expand Button */}
                      <td className="border border-slate-200 dark:border-slate-700/50 px-6 py-4 text-center">
                  ...
                  item.methods.map((method, methodIdx) =>
                    selectedMethod === `${idx}-${methodIdx}` ? (
                      <Card
                        ...
                        className="bg-white dark:bg-slate-900/70 backdrop-blur-xl border border-emerald-300/50 overflow-hidden shadow-lg shadow-emerald-200/30"
                      >
                        <div className="p-8">
                          <div className="mb-8">
                            ...
                            <p className="text-slate-700 dark:text-slate-300">{method.summary || method.description || 'No description'}</p>
                          </div>
                          ...
                          <div className="grid md:grid-cols-2 gap-8">
                            ...
                            {method.parameters && method.parameters.length > 0 && (
                              <div>
                                <h4 className="text-lg font-semibold text-slate-800 dark:text-slate-200 mb-4 flex items-center gap-2">
                                  <span className="bg-amber-100 dark:bg-amber-900/50 text-amber-700 px-3 py-1 rounded-lg text-sm">📋</span>
                                ...
                                <div className="space-y-2">
                                  {method.parameters.map((param, pIdx) => (
                                    <div
                                      key={pIdx}
                                      className="bg-white dark:bg-slate-900/60 hover:bg-white dark:bg-slate-900 border border-amber-200 dark:border-amber-800/50 hover:border-amber-400/60 rounded-lg p-4 transition-all"
                                    >
                                      ...
                                      <p className="text-slate-600 dark:text-slate-400 text-xs">
                                        Location: <span className="text-slate-700 dark:text-slate-300 font-mono">{param.in}</span>
                                      </p>
                                    </div>
                                  ))}
                                </div>
                            ...
                            {method.responses && Object.keys(method.responses).length > 0 && (
                              <div>
                                <h4 className="text-lg font-semibold text-slate-800 dark:text-slate-200 mb-4 flex items-center gap-2">
                                  <span className="bg-emerald-100 dark:bg-emerald-900/50 text-emerald-700 px-3 py-1 rounded-lg text-sm">✓</span>
                                  ...
                                  {Object.entries(method.responses).map(([statusCode, response]) => (
                                    <div
                                      ...
                                      className={`border rounded-lg p-4 hover:shadow-lg transition-all ${statusCode.startsWith('2')
                                        ? 'bg-emerald-100 dark:bg-emerald-900/50 border-emerald-300/50 hover:bg-emerald-100 dark:bg-emerald-900/50/80'
                                        : statusCode.startsWith('4')
                                          ? 'bg-amber-100 dark:bg-amber-900/50 border-amber-300 dark:border-amber-700/50 hover:bg-amber-100 dark:bg-amber-900/50/80'
