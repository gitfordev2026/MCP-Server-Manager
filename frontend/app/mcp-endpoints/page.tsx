...
function permissionBadgeClass(mode: AccessMode): string {
  switch (mode) {
    case 'allow':
      return 'bg-emerald-100 dark:bg-emerald-900/50 text-emerald-700 border-emerald-300';
    case 'deny':
      return 'bg-red-100 text-red-700 border-red-300';
    default:
      return 'bg-amber-100 dark:bg-amber-900/50 text-amber-700 border-amber-300 dark:border-amber-700';
  }
}
    ...
    <div className="min-h-screen bg-gradient-to-br from-white via-slate-50 to-slate-100 dark:from-slate-950 dark:via-slate-900 dark:to-slate-950 overflow-hidden">
      ...
      <main className="pt-24 pb-12 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto relative z-10">
        ...
        <div className="flex items-start justify-between mb-8 gap-4">
          ...
          <button
            onClick={() => fetchData()}
            className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300 px-4 py-2 rounded-lg hover:bg-slate-50 dark:bg-slate-800/50 font-semibold text-sm transition-all hover:scale-105"
          >
            Refresh
          </button>
        </div>
        ...
        {error && (
          <div className="mb-6 bg-amber-100 dark:bg-amber-900/50 border border-amber-300 dark:border-amber-700 rounded-xl p-4 text-amber-700">
        ...
        {loading ? (
          <div className="flex items-center justify-center py-20">
            ...
            <span className="ml-3 text-slate-600 dark:text-slate-400 font-medium">Loading endpoints...</span>
          </div>
        ) : (
          <>
            ...
            <div className="mb-8">
                  ...
                  <div className="p-6 bg-gradient-to-br from-white via-violet-50/30 to-cyan-50/20">
                    <div className="flex items-center justify-between mb-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <h2 className="text-xl font-bold text-slate-900 dark:text-slate-100">Combined MCP Server</h2>
                          <p className="text-sm text-slate-500 dark:text-slate-400">Only public/client-allowed tools are exposed</p>
                        </div>
                      </div>
                    ...
                    <div className="flex flex-col md:flex-row md:items-center gap-3">
                      ...
                      <div className="flex gap-4 text-sm">
                        ...
                        <div className="text-center">
                          ...
                          <p className="text-xs text-slate-500 dark:text-slate-400">Tools</p>
                        </div>
                        <div className="text-center">
              ...
              {expandedCard === 'combined' && (
                <div className="mt-3 border border-violet-200 rounded-2xl bg-white dark:bg-slate-900/90 p-5 shadow-lg animate-slideInUp">
                  ...
                  {Object.keys(combinedToolsByApp).length === 0 ? (
                    <p className="text-sm text-slate-500 dark:text-slate-400">No public tools available. Set access policy to Allow to expose tools.</p>
                  ) : (
                      ...
                      {Object.entries(combinedToolsByApp).map(([appName, tools]) => (
                        <div key={appName}>
                          <div className="flex items-center gap-2 mb-2">
                            ...
                            <h4 className="text-sm font-bold text-slate-800 dark:text-slate-200">{appName}</h4>
                              ...
                              return (
                                ...
                                >
                                  <div className="min-w-0 flex-1">
                                    <p className="text-sm font-semibold text-slate-800 dark:text-slate-200 truncate">{tool.title || tool.name}</p>
            ...
            <div className="mb-4">
              <h2 className="text-lg font-bold text-slate-800 dark:text-slate-200 mb-1">Individual MCP Servers</h2>
            ...
            {servers.length === 0 ? (
              <div className="text-center py-16 bg-white dark:bg-slate-900/60 rounded-2xl border border-dashed border-slate-300 dark:border-slate-600">
                <svg className="w-12 h-12 text-slate-300 mx-auto mb-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M5 12h14M12 5l7 7-7 7" />
                </svg>
                <p className="text-slate-500 dark:text-slate-400 font-medium">No MCP servers registered yet.</p>
                ...
                {servers.map((server) => {
                  ...
                  return (
                    <div key={server.name} className={isExpanded ? 'md:col-span-2 lg:col-span-3' : ''}>
                      ...
                      >
                        <div
                          className={`rounded-2xl border-2 transition-all duration-300 overflow-hidden ${isExpanded
                            ...
                            : 'border-slate-200 dark:border-slate-700 shadow-md hover:border-emerald-300 hover:shadow-lg hover:shadow-emerald-100/20'
                            }`}
                        >
                          ...
                          <div className="p-5 bg-white dark:bg-slate-900">
                            <div className="flex items-center justify-between mb-3">
                              <div className="flex items-center gap-3">
                                ...
                                <div>
                                  <h3 className="text-base font-bold text-slate-900 dark:text-slate-100">{server.name}</h3>
                                  <span className="px-2 py-0.5 bg-emerald-100 dark:bg-emerald-900/50 text-emerald-700 text-[10px] font-semibold rounded-full border border-emerald-200 dark:border-emerald-800">
                                    MCP SERVER
                                  </span>
                                </div>
                              </div>
                      ...
                      {isExpanded && (
                        <div className="mt-3 border border-emerald-200 dark:border-emerald-800 rounded-2xl bg-white dark:bg-slate-900/90 p-5 shadow-lg animate-slideInUp">
                          <h3 className="text-sm font-semibold text-slate-700 dark:text-slate-300 uppercase tracking-wide mb-4">
                            Tools — {server.name}
                          </h3>
                          ...
                          {toolsError && (
                            <div className="bg-red-50 dark:bg-red-900/30 border border-red-200 rounded-lg p-3 text-sm text-red-700">
                              {toolsError}
                            </div>
                          )}

                          {tools && tools.length === 0 && (
                            <p className="text-sm text-slate-500 dark:text-slate-400">No tools reported by this server.</p>
                          )}
                            ...
                            <div className="grid gap-2">
                              {tools.map((tool) => {
                                ...
                                return (
                                  <div
                                    key={tool.name}
                                    className="flex items-center justify-between p-3 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 hover:shadow-sm transition-all"
                                  >
                                    <div className="min-w-0 flex-1">
