...
export default function PlaygroundPage() {
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-white via-slate-50 to-slate-100 dark:from-slate-950 dark:via-slate-900 dark:to-slate-950 flex flex-col overflow-hidden">
            ...
            <main className="flex-1 flex flex-col pt-24 pb-4 relative z-10">
                <div className="max-w-6xl w-full mx-auto px-4 sm:px-6 lg:px-8 flex flex-col h-full gap-6">
                    ...
                    {/* Header Controls */}
                    <div className="bg-white dark:bg-slate-900/80 backdrop-blur-xl border border-rose-200/50 rounded-2xl p-6 shadow-lg shadow-rose-200/20 flex flex-col sm:flex-row items-center justify-between gap-4">
                        <div>
                        ...
                        <div className="flex items-center gap-3 w-full sm:w-auto">
                            {loading ? (
                                <div className="text-sm font-medium text-slate-500 dark:text-slate-400 animate-pulse">Loading apps...</div>
                            ) : (
                                <select
                                    value={selectedApp}
                                    onChange={(e) => handleAppSelect(e.target.value)}
                                    className="w-full sm:w-64 bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-600 text-slate-700 dark:text-slate-300 text-sm rounded-lg focus:ring-rose-500 focus:border-rose-500 block p-2.5 shadow-sm transition-all hover:border-rose-300"
                                >
                    ...
                    {/* Chat Container */}
                    <div className="flex-1 bg-white dark:bg-slate-900/70 backdrop-blur-xl border border-rose-300/40 rounded-2xl overflow-hidden flex flex-col shadow-xl shadow-orange-200/20">
                                    ...
                                    <div className={`flex gap-3 max-w-xs sm:max-w-md lg:max-w-2xl`}>
                                        ...
                                        <div
                                            className={`px-5 py-4 rounded-2xl transition-all duration-300 hover:scale-[1.02] ${message.role === 'user'
                                                    ...
                                                    : 'bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-200 rounded-bl-none border border-rose-200 shadow-md shadow-rose-100/50'
                                                }`}
                                        >
                            ...
                            {chatLoading && (
                                <div className="flex justify-start animate-slideInLeft">
                                    <div className="flex gap-3">
                                        ...
                                        <div className="bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-200 px-5 py-4 rounded-2xl rounded-bl-none border border-rose-200 shadow-md">
                        ...
                        {/* Input Area */}
                        <div className="border-t border-rose-200/50 bg-white dark:bg-slate-900/80 backdrop-blur-xl p-4 sm:p-5 shadow-lg">
                            <form onSubmit={handleSendMessage} className="flex gap-3">
                                <input
                                    ...
                                    className="flex-1 px-5 py-3 border border-slate-300 dark:border-slate-600 rounded-2xl focus:outline-none focus:ring-2 focus:ring-rose-500 focus:border-transparent bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-200 placeholder-slate-400 transition-all duration-300 hover:border-slate-400 font-medium"
            ...
            {isModalOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-fadeIn">
                    <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl max-w-2xl w-full max-h-[85vh] flex flex-col overflow-hidden animate-slideInUp border border-slate-200 dark:border-slate-700">
                        <div className="p-6 border-b border-slate-100 bg-slate-50 dark:bg-slate-800/50/50">
                        ...
                        <div className="p-4 bg-slate-100 dark:bg-slate-800/50 flex items-center justify-between border-b border-slate-200 dark:border-slate-700">
                            ...
                            <div className="flex gap-2">
                                <button
                                    ...
                                    className="text-xs font-semibold text-rose-600 bg-white dark:bg-slate-900 border border-rose-200 px-3 py-1.5 rounded-lg hover:bg-rose-50 transition-colors"
                                >
                                    Select All
                                </button>
                                <button
                                    onClick={() => setSelectedToolNames(new Set())}
                                    className="text-xs font-semibold text-slate-600 dark:text-slate-400 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 px-3 py-1.5 rounded-lg hover:bg-slate-50 dark:bg-slate-800/50 transition-colors"
                                >
                                    Deselect All
                                </button>
                            </div>
                                    ...
                                    appTools.map((tool) => (
                                        <label
                                            ...
                                            className={`flex items-start gap-3 p-4 rounded-xl cursor-pointer transition-all border ${selectedToolNames.has(tool.name)
                                                    ...
                                                    : 'bg-white dark:bg-slate-900 border-transparent hover:bg-slate-50 dark:bg-slate-800/50'
                                                }`}
                                        >
                                            <div className="pt-0.5">
                                                <input
                                                    ...
                                                    className="w-5 h-5 text-rose-500 bg-white dark:bg-slate-900 border-slate-300 dark:border-slate-600 rounded focus:ring-rose-500 focus:ring-2 cursor-pointer"
                                                />
                                            </div>
                                            <div className="flex-1">
                                                ...
                                                <p className="text-xs text-slate-400 font-mono mt-1 break-all bg-white dark:bg-slate-900 py-1 px-2 rounded border border-slate-100 inline-block">
                                                    {tool.name}
                                                </p>
                                            </div>
                                        </label>
                        ...
                        <div className="p-4 border-t border-slate-100 bg-white dark:bg-slate-900 flex justify-end gap-3 rounded-b-2xl">
                            <button
                                onClick={() => {
                                    setIsModalOpen(false);
                                    setSelectedApp('all');
