...
function getPermission(policies: Policies, ownerId: string, endpointId: string): AccessMode {
  const policy = policies[ownerId];
  if (!policy) return 'approval';
  return policy.endpointModes[endpointId] ?? policy.defaultMode ?? 'approval';
}
            ...
            <div className="mb-8">
              <button
                ...
                className={`w-full text-left group transition-all duration-300 ${
                  expandedCard === 'combined' ? '' : 'hover:scale-[1.01]'
                }`}
              >
                <div
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <svg
                          className={`w-5 h-5 text-slate-400 transition-transform duration-300 ${
                            expandedCard === 'combined' ? 'rotate-180' : ''
                          }`}
                          fill="none"
                          stroke="currentColor"
                          viewBox="0 0 24 24"
                      ...
                      {Object.entries(combinedToolsByApp).map(([appName, tools]) => (
                        <div key={appName}>
                          ...
                          <div className="grid gap-2 ml-4">
                            {tools.map((tool) => {
                              const mode = getPermission(policies, `app:${appName}`, tool.name);
                              return (
                                <div
                                  key={tool.name}
                        ...
                        <div className="mt-3 border border-emerald-200 rounded-2xl bg-white/90 p-5 shadow-lg animate-slideInUp">
                          ...
                          {tools && tools.length > 0 && (
                            <div className="grid gap-2">
                              {tools.map((tool) => {
                                const mode = getPermission(policies, `mcp:${server.name}`, tool.name);
                                return (
                                  <div
                                    key={tool.name}
